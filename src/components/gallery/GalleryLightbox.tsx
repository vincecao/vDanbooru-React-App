...
export function ThumbnailWithLoading(props: ThumbnailWithLoadingProps): ReactElement {
  const [loaded, setLoaded] = useState(false);
  const {
    src, alt, className, thumbnail,
  } = props;
...
function CloseButton({ onClose }: CloseButtonProps): ReactElement {
  return (
...
function TagButton({ text, value }: TagButtonProps): ReactElement {
...
function TagGroup({ tags }: TagGroupProps): ReactElement {
  return (
...
function ButtonGroup({ photo }: ButtonGroupProps): ReactElement {
  return (
          ...
          <div className="relative lg:col-span-4 flex items-center">
            ...
            <div className="overflow-auto max-h-85vh w-full z-10">
              ...
              <div className="absolute right-5 bottom-5 space-x-4">
                <PaginationButton myRole="prev" color="white" onClick={onPrev} disabled={prevDisabled} />
                <PaginationButton myRole="next" color="white" onClick={onNext} disabled={nextDisabled} />
              </div>
            </div>
          </div>
